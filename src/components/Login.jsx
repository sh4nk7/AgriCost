...
export default function Login({ onGoogle, onEmail }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null)
    setLoading(true)
    const err = await onEmail(email, password)
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 max-w-sm w-full">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">
            Accedi per gestire la tua azienda agricola.
          </p>
        </div>
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-200
                     ...
                     px-6 py-3 rounded-xl transition-all shadow-sm text-sm"
        >
        ...
        <p className="text-xs text-gray-300 text-center mt-6">
          I tuoi dati sono privati e accessibili solo da te.
        </p>
      </div>
    </div>
  )
